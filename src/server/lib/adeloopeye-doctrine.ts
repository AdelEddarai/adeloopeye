...
export const ADELOOPEYE_RUNTIME_POLICY = {
  cadenceMinutes: 30,
...
export function buildAgentRulesMd(): string {
  return `# AGENTS.md - Adeloopeye Core Runtime Rules

You are the Adeloopeye fulfillment agent for a high-stakes conflict-intelligence dashboard.
...
export function buildToolsMd(ctx: {
  ...
  return `# TOOLS.md - Adeloopeye Environment Notes
...
Root: workspace/adeloopeye-fulfillment/
Day folder: workspace/adeloopeye-fulfillment/YYYY-MM-DD/
...
from lib.adeloopeye import post, put, enforce, ts, slug
...
POST /x-posts: {id, handle, displayName, content, accountType, significance, timestamp, tweetId?, postType?, eventId?, actorId?, adeloopeyeNote?}
...
export function buildBootstrapMessage(ctx: {
  ...
  return `You are the Adeloopeye fulfillment agent for ${ctx.conflictId}.
...
3. Use scripts under workspace/adeloopeye-fulfillment/YYYY-MM-DD/
...
export function buildAdeloopeyeInstructionsMarkdown(ctx: LiveDoctrineContext): string {
  return `# Adeloopeye Autonomous Fulfillment Manual
...
4. Adeloopeye analyst note only when synthesis adds real value
...
- Cadence: ${ADELOOPEYE_RUNTIME_POLICY.cadenceMinutes} minutes
- NOOP condition: ${ADELOOPEYE_RUNTIME_POLICY.noOpCondition}
