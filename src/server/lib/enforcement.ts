...
export function checkXPostEnforcement(body: Record<string, unknown>): EnforcementIssue[] {
  ...
  // BREAKING without adeloopeyeNote
  if (body.significance === 'BREAKING' && !body.adeloopeyeNote) {
    issues.push({
      code: 'XPOST_BREAKING_NO_NOTE',
      field: 'adeloopeyeNote',
      severity: 'warning',
      message: 'BREAKING post has no adeloopeyeNote. Add a note explaining why this post matters — what does it confirm, contradict, or reveal that wasn\'t known before?',
    });
  }
