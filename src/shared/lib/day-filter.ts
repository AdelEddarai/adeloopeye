 ...
 */
export function getActorForDay(actor: Actor, day: string, allDays?: string[]): ActorDaySnapshot | null {
  // Safety check: if daySnapshots is undefined or empty, return null
  if (!actor.daySnapshots || Object.keys(actor.daySnapshots).length === 0) {
    return null;
  }
  ...
  return actor.daySnapshots[keys[keys.length - 1]] || null;
}
