...
import { EmbedSkeleton, EngStat, AdeloopeyeNote, VerificationBadge } from './x-post-subcomponents';

// Adeloopeye intel view (reused in both layouts)
...
export function AdeloopeyeView({ post, acct, postUrl, compact }: Props) {
  ...
  return (
    <>
      ...
      {/* ── ADELOOPEYE NOTE ── */}
      {!compact && post.adeloopeyeNote && (
        <AdeloopeyeNote note={post.adeloopeyeNote} />
      )}
    </>
  );
}
