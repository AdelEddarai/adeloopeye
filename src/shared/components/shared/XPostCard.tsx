...
import { EmbedSkeleton, AdeloopeyeView } from './AdeloopeyeView';
        ...
        <div className="grid h-[min(78vh,760px)] min-h-[640px] min-w-0 grid-cols-1 grid-rows-[minmax(0,1fr)_minmax(0,1fr)] overflow-hidden lg:h-auto lg:min-h-0 lg:grid-cols-[1fr_1fr] lg:grid-rows-1">
          ...
          <div className="flex min-h-0 min-w-0 flex-col overflow-y-auto">
            <div className="flex items-center gap-1 px-3 py-1.5">
              <span className="mono text-[length:var(--text-tiny)] text-[var(--blue-l)] tracking-[0.08em]">ADELOOPEYE INTEL</span>
            </div>
            <AdeloopeyeView post={post} acct={acct} postUrl={postUrl} />
          </div>
        </div>
      ) : <AdeloopeyeView post={post} acct={acct} postUrl={postUrl} compact={compact} />}
    </div>
  );
}
