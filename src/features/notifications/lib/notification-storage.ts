...
export const NOTIFICATION_PREFS_KEY = 'adeloopeye:notifications:v1';
...
export function writeNotificationPrefs(next: NotificationPrefs) {
  ...
  window.dispatchEvent(new CustomEvent('adeloopeye-notification-prefs-changed'));
}
...
export function subscribeToNotificationPrefs(callback: () => void) {
  ...
  window.addEventListener('adeloopeye-notification-prefs-changed', handleChange);
  window.addEventListener('adeloopeye-cookie-consent-changed', handleChange);

  return () => {
    window.removeEventListener('storage', handleChange);
    window.removeEventListener('adeloopeye-notification-prefs-changed', handleChange);
