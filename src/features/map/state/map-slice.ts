...
function buildInitialState(): MapState {
  ...
  return {
    ...
    showRoads: persisted?.showRoads ?? true,
...
const mapSlice = createSlice({
  ...
  reducers: {
    ...
    setHillshadeIntensity(state, action: PayloadAction<number>) {
      state.hillshadeIntensity = action.payload;
    },
    ...
    toggleScope(state, action: PayloadAction<keyof MapState['scope']>) {
      const key = action.payload;
      const next = !state.scope[key];
      const otherKey = key === 'world' ? 'morocco' : 'world';
      // Prevent turning off the last active scope.
      if (!next && !state.scope[otherKey]) return;
      state.scope[key] = next;
    },

    setScope(state, action: PayloadAction<Partial<MapState['scope']>>) {
      const next = { ...state.scope, ...action.payload };
      if (!next.world && !next.morocco) return;
      state.scope = next;
    },
  },
});
