...
const CATEGORY_COLORS: Record<string, { bg: string; text: string }> = {
  ...
  INTELLIGENCE:{ bg: 'rgba(160,100,220,0.12)', text: 'var(--cyber)' },
  DIPLOMATIC:  { bg: 'var(--success-dim)', text: 'var(--success)' },
  CYBER:       { bg: 'rgba(220,50,220,0.12)', text: 'var(--danger)' },
  BREAKING:    { bg: 'var(--warning-dim)', text: 'var(--warning)' },
  DEFAULT:     { bg: 'var(--bg-2)', text: 'var(--t3)' },
};
...
export function StoryCard({ story, isOpen, onToggle, onFlyTo }: Props) {
  const catColor = CATEGORY_COLORS[story.category] || CATEGORY_COLORS.DEFAULT;
