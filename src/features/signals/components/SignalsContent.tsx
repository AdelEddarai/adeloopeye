...
export function SignalsContent() {
  ...
  const [adeloopeyeOnly, setAdeloopeyeOnly] = useState(false);
  ...
  const filtered = useMemo(() => {
    ...
    return base.filter(p => {
      ...
      if (adeloopeyeOnly && !p.adeloopeyeNote)              return false;
      if (verifiedOnly && p.verificationStatus !== 'VERIFIED') return false;
      return true;
    });
  }, [sigFilter, acctFilter, adeloopeyeOnly, verifiedOnly, currentDay, showAll, allPosts, allDays]);
  ...
  const filterRail = (
    <SignalFilterRail
      ...
      onAdeloopeyeOnly={v => { setAdeloopeyeOnly(v); }}
