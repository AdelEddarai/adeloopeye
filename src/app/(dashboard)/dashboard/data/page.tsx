    ...
    >
      <div className="max-w-4xl mx-auto">
        <div className="flex items-baseline gap-3 mb-6">
          ...
          <span className="text-[length:var(--text-label)] text-[var(--t4)]">Open-source intelligence feeds</span>
        </div>
          ...
          {DATA_SOURCES.map(source => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="mono text-[length:var(--text-body-sm)] font-bold text-[var(--t1)] tracking-wider">
                  {source.label}
                </span>
                 <span className={`ml-auto px-1.5 py-0.5 text-[length:var(--text-micro)] mono font-bold border ${
                   source.status === 'LIVE'
              ...
              <p className="text-[length:var(--text-label)] text-[var(--t3)] leading-relaxed mb-2">
                {source.description}
              </p>
              <span className="mono text-[length:var(--text-caption)] text-[var(--t4)]">{source.count}</span>
            </Link>
          ))}
        </div>
      </div>
